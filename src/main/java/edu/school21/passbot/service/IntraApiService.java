...
public class IntraApiService {
    ...
    public User requestAccessToken(String name) throws IOException {
        ...
        response =
            restTemplate.exchange(access_token_url, HttpMethod.POST, request,
                String.class);
        ...
        while (node2.hasNext()) {
            ...
            if (tmp.getKey().equals("staff?")) {
                user.setRole(
                    tmp.getValue().asText().equals("false") ? "USER" : "ADMIN");
            }
        ...
        ResponseEntity<String> peers2 =
            restTemplate.exchange(url_campus, HttpMethod.GET, entity1,
                String.class);
