...
public class PassBot extends TelegramLongPollingBot {
    ...
    public PassBot(PassBotConfig config, CommandsFactory commandsFactory,
                   CallbackQueryFacade callbackHandlerFacade,
                   UserDataCache usersDataCache) {
        this.config = config;
    ...
    public void sendMessage(SendMessage message) {
        try {
            ...
            log.error("Failed to send message \"{}\"", message.toString());
            e.printStackTrace();
        }
    }
    ...
    public void sendUpdate(BotApiMethod<Serializable> update) {
        try {
            execute(update);
            log.info("Update sent \"{}\" ", update.toString());
        } catch (TelegramApiException e) {
            log.error("Failed to send update \"{}\"", update.toString());
            e.printStackTrace();
        }
    }
    ...
    private void setCommands() {
        ...
        commandsList.add(
            new BotCommand("start",
                "запустить бота и ввести свой ник в интре"));
        commandsList.add(
        ...
        commandsList.add(
            new BotCommand("listall",
                "показать все активные заявки (только для сотрудников школы)"));
        commandsList.add(
