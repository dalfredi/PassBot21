...
public class Renderer {
    ...
    public static List<SendMessage> plainMessage(@NotNull Long chatId,
                                                 @NotNull String text) {
    ...
    public static List<SendMessage> toAdminOrderCards(Long chatId,
                                                      List<Order> orders) {
        ...
        for (Order order : orders) {
            ...
            response.setReplyMarkup(
                InlineKeyboards.orderCardAdmin(order.getId()));
    ...
    public static List<SendMessage> toUserOrderCards(Long chatId,
                                                     List<Order> orders) {
        List<SendMessage> sendMessageList = new LinkedList<>();
        for (Order order : orders) {
            SendMessage response = new SendMessage();
            ...
            response.setText(order.toMarkdownPrettyString());
            if (order.getStatus()
                .equals(Order.OrderStatus.PROCESSING.getText())) {
                response.setReplyMarkup(
                    InlineKeyboards.orderCardUser(order.getId()));
            }
            response.enableMarkdown(true);
            sendMessageList.add(response);
        }
    ...
    public static List<SendMessage> statusChangedMessage(Long chatId,
                                                         Order order) {
        String status = order.getStatus();
        String emojiStatus =
            status + (status.equals(Order.OrderStatus.APPROVED.getText())
                      ? APPROVE_POSTFIX
                      : DECLINE_POSTFIX);
        SendMessage response = new SendMessage();
        response.setChatId(chatId);
